"""
Implementation of a Learner using Q-Learning as learning algorithm.
...
import argparse
from algorithms.Q_Learning import QL

def learn_trajectories(env, n_batch, traj_len, n_learner_steps, gamma=0.96, learning_rate=None, epsilon=0.9):
    ...
    for step in range(n_learner_steps):
        ...
        q_learning = QL(Q, env.size ** 2, 4, gamma, p=p, r=r, epsilon=epsilon)
        param_k, Q = q_learning.update()
...
if __name__ == '__main__':
    ...
    if args.dir_traj == None:
        dir_traj = '../agent_trajectories/QL'
    else:
    ...
    trajectories, feat_rewards, params, discounted_return = learn_trajectories(env, args.batch_size, args.len_traj,
                                                           ...
                                                           args.learning_rate)
    np.save(dir_traj + '/traj_QL_' +str(args.expe) + '.npy', trajectories)
    np.save(dir_traj + '/feat_QL_' +str(args.expe) + '.npy', feat_rewards)
    np.save(dir_traj + '/params_QL_' +str(args.expe) + '.npy', params)
    np.save(dir_traj + '/disc_ret_QL_' + str(args.expe) + '.npy', discounted_return)
